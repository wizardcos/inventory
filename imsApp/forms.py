...
class SaveInvoice(forms.ModelForm):
    ...
    order_type = forms.CharField(max_length=100)
    product_name = forms.CharField(max_length=250)
    quantity = forms.CharField(max_length=100)
    customer_code = forms.CharField(max_length=100)
    description = forms.CharField(max_length=500)
    total = forms.FloatField()

    class Meta:
        model = Invoice
        fields = ('transaction', 'customer', 'customer_code', 'order_type', 'product_name', 'quantity', 'total','description')
...
class PoleTransactionForm(forms.ModelForm):
    ...
    class Meta:
        model = PoleTransaction
        fields = '__all__'

    def clean_base_plate_size(self):
        value = self.cleaned_data.get('base_plate_size')
       
        return value

    def clean_base_plate_hole_quantity(self):
        value = self.cleaned_data.get('base_plate_hole_quantity')
        
        return value

    def clean_base_plate_thickness(self):
        value = self.cleaned_data.get('base_plate_thickness')
       
        return value

    def clean_jbolt_size(self):
        value = self.cleaned_data.get('jbolt_size')

        return value

    def clean_jbolt_thread(self):
        value = self.cleaned_data.get('jbolt_thread')
       
        return value

    def clean_jbolt_bend(self):
        value = self.cleaned_data.get('jbolt_bend')
